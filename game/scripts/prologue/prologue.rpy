# The Entirity of the Code for the Prologue
# Main Writer: SonuTheNecro
# Images: Google/Images
# Audio: freesound.org + Audacity Editing :)
define mcde = Character("McDonalds Cashier")
define sk = Character("Shadow Kody")

label prologue:
    # The Prologue Scene
    stop music
    scene carontheroad with dissolve:
        xzoom 1.8
        yzoom 1.3
    if persistent.ch00:
        $ config.rollback_enabled = True
    questionmark "Chapter 0: Prologue"
    questionmark "The Creation of ???"
    play music "audio/music/prologue/new_days.ogg" loop
    k "Holy shit Thang. I am so excited for this"
    k "You actually made my day with this trip I am so excited!"

    t "I am only doing this because Daddi forced me."
    t "You are actually a fatass..."
    t "You eat everything on spot like Kirby bro"

    k "Ayo man, that is not fair."
    k "I am trying to gain some muscle and I need that protein"

    t "..."
    t "..."
    t "You realize it is really hard to gain muscle while also trying to lose weight..."
    t "You are actually a dumb fuck, shut up so we can get there without me getting a headache"
    k "I will fuck you up, I am the goat and god of arm-wrestling!"
    t "No one asked for your opinion"
    t "also good luck having some lame ass muscles compared to my gun because i will shoot you"
    k "you don't have a gun and you wouldn't shoot me"
    t "if you keep talking, I will I swear to god"
    scene mcdonalds_outside with dissolve:
        xzoom 3.2 yzoom 2.6

    show thanga2 with moveinleft:
        xzoom 1.3 yzoom 1.3 xalign 0.3 yalign 0.9
    show kody with moveinright:
        subpixel True xrotate 0.0 yrotate 180.0 xzoom 1.3 yzoom 1.3 xalign 0.8 yalign 0.9
    
    t "well alright"
    t "here we are... the worst place in America"
    k "OMG YES I LOVE YOU THANG YOU ARE THE BEST!"
    k "TIME TO EAT!!"
    
    show thanga2:
        linear 1 pos (0.04, 0.9) 

    t "What the fuck?"
    t "Are you like gay or something?"
    k "BRO I AIN'T GAY I JUST LOVE MCDONALDS NOW LETS GO I AM HUNGRY"
    t "you are gay"
    k "FUCK OFF!"
    k "I WILL BEAT YOU OFF!"
    t "..."
    k "..."
    k "wording"
    t "yeah I know"
    scene mcdonalds_inside with fade:
        xzoom 1.9
        yzoom 1.9

    t "Alright what do you even want to eat?"
    k "uhhhhhh"
    #k "I'll have two number nines"
    #k "a number nine large"
    #k "a number forty five"
    #k "a number si-"
    k "I think I will take 29 Big Macs"
    k "17 Large French Fries"
    k "7 McChickens"
    k "14 Mc IceCreams"
    k "55 Apple Slices"
    t "NO"
    t "Stop"
    t "..."
    t "Make a real order or I'm taking you home"
    k "This is my real order"
    t "I thought you were trying to be healthy and lose weight"
    k "yeah that's why I got apple slices"
    t "55?"
    t "That cannot be healthy.."
    t "also you got 29 big macs!"
    t "That's worse than Nikocado Avocado"
    #k "I did not say I wanted to be healthy"
    #k "All I said was I wanted to lose weight"
    #t "Well you have to pick a healthy option in order to do that"
    #k "Wait what..."
    #"A very lengthy lecture about Health Nutritution is pain stakingly yelled by Thanga towards Kody"
    k "Ok ok fine."
    k "I'll have the McChicken Salad and an Oreo Shake IceCream"
    t "I don't even care enough anymore, just do whatever"
    show brian_mcdonalds with dissolve:
        subpixel True crop_relative True pos (450, 223) crop (0.0, -0.11, 1.0, 1.0) 

    mcde "Hello, welcome to McDonalds! May I take your order?"

    stn "wait..."
    stn "This isn't Chick Fil-A!"
    stn "HENRY!"
    stn "Why did you write it like this?"
    tv "Ask ChatGPT"
    stn "no no no"
    stn "We can't have nice normal people in McDonalds working"
    stn "Lemme rewrite this shit real quick"
    "..."
    "..."
    "..."

    mcde "What do you fuckers want?"
    menu:
        "Big Mac":
            t "Uh hi, I would like two Big Macs"
            mcde "fatasses"
        "McChicken Salad":
            t "Do you guys still have the McChicken Salad"
            mcde "of course we do you fucker"
            t "Alright Can I have one of those for my mentally challenged brother and I will take a Big Mac for myself"
        "Fries":
            t "Alright Can I have just some fries and a Big Mac?"
            mcde "Dang, you aren't gonna order 29 big macs for the fatass next to you"
            t "you are telling me..."

    k "You forgot the Icecream :("
    mcde "Anything else you fucker?"
    t "Uh can I please get an Oreo Shake IceCream"
    stop music
    mcde "You fucking ass stupid ass muthafucka"
    play music "audio/music/prologue/mayonaka_tv.ogg" loop
    mcde "The IceCream Machine is..."
    mcde "FUCKING BROKEN!"
    mcde "L BOZO!"

    #Denial
    k "What..."
    k "..."
    k "..."
    k "..."
    k "..."
    k "..."
    k "WHAT?!?!?"
    k "No.... This cannot be"
    #Anger
    k "WHY DON'T YOU GO FIX IT YOU FUCKER!"
    k "I SAID FIX IT, YOU HEAR ME YOU BITCH"
    mcde "not really my problem lol"
    t "Kody shut up, he isn't going to do it if you are going to be an asshat in public. "
    #Bargaining
    k "please fix it, I will give my best roblox cosmetic."
    k "It is so great..."
    t "???"
    mcde "I don't get paid enough for this dumb shit."
    #Depression
    play sound "audio/sound/prologue/clock1.ogg"
    "Some time passes"
    k "My life is over..."
    k "The McDonalds Ice Cream was my pride and joy."
    k "and it is all gone..."
    t "?????"
    #Acceptance
    k "Okay whatever..."
    k "Can I just have 3 Big Macs..."
    k "..."
    k "Wait Thang where did he go?"
    t "You are flipping out so hard he is off shift now... "
    t "It is now 6 pm..."
    k "what time did we leave home?"
    t "We left to get lunch..."
    k "I don't eat lunch at school"
    t "that explains so much but so little..."
    t "where does your lunch money go?"
    k "roblox cosmetics"
    t "OF COURSE"
    t "WHAT ELSE WOULD IT BE FOR?"
    t "I AM GLAD I WASTED MY TIME FOR THIS SHIT!"
    #t "What a great way to spend my time..."

    scene kodybrain with dissolve
    show cody:
        yalign 0.5 xalign 0.5 xzoom 1.3 yzoom 1.3
    call dio_time_stop
    stop music
    play music "audio/music/prologue/percieve.ogg" loop
    questionmark "WAIT WE ARE LOSING THE BIG MAC?!?"
    questionmark "NOT ON MY WATCH!"
    questionmark "I WANT THAT BIG MAC!"
    show kody:
        xalign 0.2 yalign 0.5 xzoom 1.3 yzoom 1.3
    show cody:         
        linear 0.1 xalign 0.8

    k "Huh?"
    k "Who are you?"
    questionmark "I am the true self."
    questionmark "You are me..."
    questionmark "but I am you..."
    questionmark "They call me Shadow Kody"
    play sound "audio/sound/prologue/evil_laugh.ogg"
    sk "MWAHAHAHAHAAHAHAHAHAH"
    $ renpy.pause(2.0)
    sk "*Cough* *Cough*"
    sk "HAHAHAAHAHA"
    sk "ha... ha"

    #"..."
    #"..."
    #"..."

    #tv "Oh hell nah. I'm hitting pause on this"
    #tv "BHAARAT! Why did you put a Persona 4 Reference in here..."
    #stn "idk lol"
    #tv "I'm changing him to Cody with a C because that isn't confusing one bit"

    #"..."
    sk "..."
    sk "..."
    $ rngint = renpy.random.randint(0,1)
    if rngint == 0:
        sk "Why am I the evil one?"
        sk "Whoever wrote this..."
        sk "I am clearly the one of justness"
        k "that isn't a word"
        sk "shut up no one asked you"
        sk "What if you are evil?"
        sk "and not me?"
        k "well that's just not true"
        sk "..."
        sk "no but its just a hypothesis"
        sk "just work with me on this one"
        k "no not really I just want my big mac and go home so I can play smash bros and watch anime"
        sk "i hate you"
        sk "I want nothing from you degenerate ass"
        sk "I think I will change my name to Cody with a C"
        k "Like CUM?"
        c "I really"
        c "really..."
        c "really fucking hate you"
        c "goodbye forever Kody"
    else:
        sk "Shadow implies that I am a weaker and more evil verison of you"
        sk "But in reality, I am a perfect verison of you.\n I can do things that not even gods can do!"
        sk "I think I am more worthy of your name, I think I will be taking the name Kody and lets see here..."
        c  "I think I will swap it to a C because C is #1."
        k "Wouldn't that imply that you are number #3 and not number #1..."
        c "..."
        k "Y'know, since C is the third letter of the Alphabet"
        c "Shut the fuck up"
        c "You were always the weak part of me..."
        c "I think its time if we made some changes around here."
        c "goodbye forver kody"
    stop music
    scene mcdonalds_playplace with fade:
        xalign 0.8 xzoom 3.9 yzoom 3.3
    k "where the hell am i?"

    show kody:
        xalign 0.2 yalign 0.3 xzoom 1.3 yzoom 1.3
    k "Oh this is the playplace..."
    k "wait this mcdonalds has one?"
    k "these still exist in McDonalds!?"
    k "anyways..."
    k "..."
    k "Where did that gremlin go behind you"
    questionmark "Right behind you"

    show cody:
        linear 0.8 xalign 1.3

    c "SEE YOU LATER SUCKER!"
    k "OH NO YOU DON'T!"
    k "I am the real Kody, I'm not letting some Suspicious Imposter impose as me"
    hide kody
    show kody:         
        xalign 0.2 yalign 0.3 xzoom 1.3 yzoom 1.3 
        linear 0.5 xalign 1.3

    $ rngint1 = renpy.random.randint(1,2)
    play music "audio/music/prologue/you're_my_brother.ogg"
    if rngint1 == 1:
        scene wild_west1 with fade:
            xzoom 3.9 yzoom 3.2 yalign 0.8
    else:
        scene wild_west2 with fade:
            xzoom 3.9 yzoom 3.2 yalign 0.8

    show cody with moveinright:         
        xalign 0.8 yalign 0.3 xzoom 1.3 yzoom 1.3
    c "Wait..."
    c "This isn't Harvard University Campus..."
    c "This is the WILD WEST!"
    c "I'm like an indiana Jones cowboy this is awesome"
    questionmark "Holy crap..."
    questionmark "Why is this area so hot"
    c "Ugh, I have to deal with this guy..."
    show kody with moveinleft:
        xalign 0.2 yalign 0.3 xzoom 1.3 yzoom 1.3
    k "1v1 swordpvp roblox anticheat"
    c "..."
    k "i got a swordburst 3 lobby come on kid"
    c "AND IM SUPPOSED TO BE THE VILLIAN?"
    c "WITH THIS TRASH?"
    c "can we do something fucking normal"
    k "You want to arm wrestle?"
    c "???"
    c "You realize I am going to cook you like you are some meatball and I am the fucking goat lol"
    "The battle begins now! Make your choice now!"
    $ count = 0
    jump chapter_zero_battle

label chapter_zero_ending:
    k "What a fucking dick..."
    k "I assume I am the supreme Kody now..."
    k "wait..."
    k "My BigMac!"
    show kody:
        matrixcolor SaturationMatrix(0)
    if rngint1 == 1:
        show wild_west1 with dissolve:
            xzoom 3.9 yzoom 3.2 yalign 0.8 matrixcolor SaturationMatrix(0)
    else:
        show wild_west2 with dissolve:
            xzoom 3.9 yzoom 3.2 yalign 0.8 matrixcolor SaturationMatrix(0)
    "Should..."
    "Should we stop cody?"
    "He seems kinda like a menace..."
    questionmark "No we currently need him right now..."
    "Uh... Of course... guy?"
    "And so the being Kody has split into two seperate beings"
    "and Kody has thorted Cody and kept the peace in the town"
    "or did he?"
    $ persistent.ch00 = True
    stop music
    $ renpy.quit()
    jump chapter_one
